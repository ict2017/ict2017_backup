...
if (isset($_GET['sn']))
        ...
        die('Missing Squad Number');
	
	$sql = "SELECT * FROM player left outer join playfor on player.id = playfor.id left outer join team on team.teamid = playfor.teamid WHERE playfor.squadnum = '$squadnum' ORDER BY player.id ";

	$result = $dbh->prepare($sql);
	$result->execute();
	if(!$result->fetch(PDO::FETCH_ASSOC))
	    die('Invalid Squad Number or There Is No Player With That Squad Number.');
	$result->execute();
	echo " <b>Listing Player(s) With Squad Number $squadnum :</b> <br><br>";
?>
